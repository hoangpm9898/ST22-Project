...
async function fetchWallpaperDetail(wallpaperId) {
  try {
    ...
    if (provider==='seaart.ai') {
      ...
      console.log(`Fetch wallpaper detail (to download):`, wallpaperId);
      ...
      if (result && result.data) return result.data.banner.url;
    }
  } catch (error) {
...
async function updateWallpaperStatus(index, wallpaperData, isPopup, flowType) {
  ...
  if (result) {
    ...
    else if (flowType==='add-profile') {
      allWallpapers[currentImageIndex].addedProfile = true;
    }
...
function showPreviewPopup(index, inputWallpaperDataId) {
  ...
  const addedTo = wallpaper.addedAlbum || wallpaper.addedProfile;

  // Thêm remove-overlay nếu đã xóa
  if (isRemoved || addedTo) {
    const removeOverlay = document.createElement('div');
    removeOverlay.className = 'remove-overlay active';
    removeOverlay.innerHTML = `<span>Wallpaper has been ${(isRemoved)?'removed':'added to album/profile'}!</span>`;
    viewPopup.appendChild(removeOverlay);
  }
...
async function renderWallpapers(pageNum = 1) {
  ...
  if (data && data.length>0) {
    ...
    const wallpapersWithStatus = data.map(wallpaper => ({
      ...
      addedProfile: false,  // status added to profile
    }));
...
async function initializeEventListeners() {
  ...
  document.getElementById('providerSelect').value = 'seaart.ai';
