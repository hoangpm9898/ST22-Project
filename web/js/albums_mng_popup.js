...
async function verifyRequest(type, id) {
    try {
        ...
        const apiURL = `${API_HOST_URL}/api/${type}/verify?${type}Id=${id}`;
...
document.getElementById('album-list').addEventListener('click', async (e) => {
    ...
    if (albumItem) {
        ...
        document.querySelectorAll('#album-list .album-item').forEach(item => {
...
verifyAlbumBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        // Verify...
        const verifyData = await verifyRequest('album', albumId);
